...
def build_recommendations(title):
    try:
        ...
        if imdb.iloc[idx]['titleType'] in tv_shows:
            ...
            return pd.DataFrame(tv_recommendations).transpose().iloc[1:16]
            ...
            return pd.DataFrame(movie_recommendations).transpose().iloc[1:16]
    except:
        return None
...
def get_recommendations(title):
    ...
    if recommendations is None:
        ...
        # Show Ratings
        recommendations['rating']=imdb.loc[recommendations.index, "averageRating"].values
...
def adapt_recommendation(recommendations, user_preferences):
    if not user_preferences["selected_movies"]:
        return recommendations
    
    for idx, row in recommendations.iterrows():
        boost=user_preferences["selected_movies"].get(row["title"],0)
        recommendations.at[idx,"rating"]+=boost*0.2
